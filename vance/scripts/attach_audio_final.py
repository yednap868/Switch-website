#!/usr/bin/env python3
"""
Script to attach audio by searching ElevenLabs API with user_id parameter.
...
async def get_conversations_by_user_id_search(phone: str, exclude_conv_ids: set) -> list[dict]:
    """Get conversations from ElevenLabs by searching with user_id parameter."""
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            # Search by user_id parameter
            print(f"   Searching with user_id={phone_normalized}...")
            params = {"user_id": phone_normalized}
            ...
            print(f"   Found {len(conversations)} conversations for user_id={phone_normalized}")
            
            # Filter out Saurabh's conversations and verify user_id
            for conv in conversations:
                if isinstance(conv, dict):
                    ...
                    if conv_id in exclude_conv_ids:
                        print(f"   ⚠️  Skipping Saurabh's conversation: {conv_id}")
                        continue
                    
                    # Verify user_id by fetching details (to be extra sure)
                    try:
                        ...
                        if user_id == phone_normalized:
                            ...
                            print(f"   ✅ Verified: {conv_id} (User ID: {user_id})")
                        else:
                            print(f"   ⚠️  User ID mismatch: {conv_id} (expected {phone_normalized}, got {user_id})")
                    except Exception as e:
                        print(f"   ⚠️  Error verifying {conv_id}: {e}")
...
async def attach_audio_final(slug: str):
    """Attach audio by searching ElevenLabs with user_id parameter."""
    separator = "=" * 70
    print(f"\n{separator}")
    ...
    # Get conversations from ElevenLabs by searching with user_id
    print(f"\n1. Searching ElevenLabs for conversations with user_id={phone}...")
    matching_conversations = await get_conversations_by_user_id_search(phone, saurabh_conv_ids)
    
    if not matching_conversations:
        print(f"   ❌ No conversations found with user_id = {phone}")
        return False
...
async def main():
    ...
    for slug in slugs:
        result = await attach_audio_final(slug)
