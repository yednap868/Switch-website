#!/usr/bin/env python3
"""
Script to attach audio by matching ElevenLabs User ID (which is WhatsApp phone number)
to the user's WhatsApp number.
"""
import sys
import os
import asyncio
...
async def get_conversations_by_user_id(phone: str) -> list[dict]:
    """Get conversations from ElevenLabs API where User ID matches the phone number."""
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Filter by User ID (which is the WhatsApp phone number)
            for conv in conversations:
                if isinstance(conv, dict):
                    ...
                    # Check User ID field (this is the WhatsApp phone number)
                    user_id = str(conv.get("user_id", "")).replace("+", "").replace("-", "").replace(" ", "")
                    ...
                    if user_id == phone_normalized:
                        ...
                        print(f"   ✅ Matched: {conv_id} (User ID: {user_id})")
                    else:
                        # Debug: show what User ID we found
                        if user_id:
                            print(f"   ⚠️  Skipping {conv_id} - User ID: {user_id} (expected: {phone_normalized})")
...
async def attach_audio_by_user_id(slug: str):
    ...
    matching_conversations = await get_conversations_by_user_id(phone)
...
async def main():
    ...
    for slug in slugs:
        result = await attach_audio_by_user_id(slug)
