#!/usr/bin/env python3
"""
Script to attach audio by fetching individual conversation details to get user_id,
which matches the WhatsApp phone number.
"""
import sys
import os
import asyncio
...
async def get_conversations_by_fetching_user_id(phone: str) -> list[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            print(f"   Checking each conversation's user_id (this may take a moment)...")
                    ...
                    try:
                        ...
                        if user_id == phone_normalized:
                            ...
                            if i <= 5 or i % 10 == 0:  # Show progress for first 5 and every 10th
                                print(f"   [{i}/{len(conversations)}] Checking {conv_id}...")
...
async def attach_audio_by_user_id(slug: str):
    ...
    matching_conversations = await get_conversations_by_fetching_user_id(phone)
